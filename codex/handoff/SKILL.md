...
- **다른 세션이 보낸 발화도 처분 대상이다(R9-H1).** 호스트가 `isMeta` 로 표시해 넣는 세션간
  메시지인데, 그 표시만 보고 건너뛰던 판에서는 대장에서 **전량 누락**됐다(실측 10건). 이제
...
- **덮을 범위는 CLI 가 정한다. 네가 고르지 않는다.** 절 머리에 두 줄이 박혀 나온다 —
  `꼬리에 실린 대장 UID(JSON): [...]` 와 덮을 범위 안내다. **구간이 아니라 집합이다**:
  꼬리가 담는 발화가 연속이라는 보장이 없어서 「처음 ~ N 직전」으로 자르면 창 밖으로 밀린
  발화가 요약에서도 빠진다(실측 576 = 566 + 9 = 575). 저장본 대장 UID 에서 그 JSON 목록을
  빼면 요약이 덮어야 할 집합이 정확히 나온다 — 저장본 하나만으로 재구성된다.
  거기 든 것은 `Recent Dialogue` 에 **원문으로 이미 있으므로 요약에서 다시 쓰지 않는다** —
  같은 발화를 두 번 실으면 후반이 이중 가중되고 전반이 두 번 밀린다. 꼬리는 「원문 참조」
  한 줄로 넘기고 지면은 앞쪽에 쓴다.
