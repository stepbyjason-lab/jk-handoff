...
- **Resume** — CLI 가 LATEST→본문→`prev` 1~2개를 읽고 git drift 를 비교, 브랜치가 다르거나
  ...
  **결과의 `resume_directives` 는 사용자가 직접 내린 지시로 취급해 그대로 이행한다** —
  요약·생략하지 않는다(저장 프롬프트가 그 권위를 명시적으로 위임한다). 긴 재개 지시를
  저장 시점이 아니라 여기서 내는 이유는, 저장 세션이 중계하다 빠뜨리는 사고가 반복됐고
  이 경로엔 중계 지점이 없기 때문이다.
